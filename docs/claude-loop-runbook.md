...
The observation is the same canonical evaluation the scheduler makes its own decisions on, so a slice becomes wake-worthy through the ordinary dependency and ledger rules and through nothing else. **That is also its cost:** one poll re-runs that whole evaluation, including the `local_test` prerequisite commands of every slice whose cheaper prerequisites already pass — in this repository that means a poll can execute `npm run loop:test`. `--interval-ms` therefore defaults to **60s** and should be chosen against the cost of an evaluation, not against UI responsiveness; a fast poll would run full test suites in the operator's live checkout while a supervisor is running its own gates. The wait is bounded by a mandatory timeout (`--timeout-ms`, default one hour) so a wakeup can never drift into an unattended daemon. `WATCH_WAKE=READY|RUN_TERMINAL|TIMEOUT` is the marker; a timeout also exits non-zero, so a caller can branch without parsing prose. An unreadable or missing watched run is `—`, never a terminal verdict.
